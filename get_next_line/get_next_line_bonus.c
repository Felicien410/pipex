...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/02 15:04:50 by fcatteau          #+#    #+#             */
/*   Updated: 2022/12/08 14:32:15 by fcatteau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buf)
		return (NULL);
	all_line = recup_the_total_row (fd, buf, stack[fd]);
	free (buf);
	buf = NULL;
	if (!all_line)
		return (NULL);
	left = get_left (all_line);
	stack[fd] = get_right (all_line);
	free (all_line);
	all_line = NULL;
	return (left);
}
